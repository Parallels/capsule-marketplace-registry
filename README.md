# Capsule Marketplace Registry Service

This directory contains the Capsule Marketplace Registry service, which handles capsule blueprints and downloads.
...
```bash
# Build the image
docker build -f Dockerfile -t capsule-marketplace-registry .

# Run the container
docker run -d \
  --name capsule-marketplace-registry \
  -p 8081:8080 \
  -v ./data:/app/data \
  -v ./logs:/app/logs \
  -v ./capsules:/app/capsules \
  capsule-marketplace-registry
```
...
docker-compose logs -f capsule-marketplace-registry
```
...
```bash
# Enter the container
docker-compose exec capsule-marketplace-registry sh
